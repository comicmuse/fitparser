...
import 'package:mobile/widgets/block_card.dart';
...
WorkoutBlock _makeBlock({
  ...
  double? targetHigh,
    ...
    WorkoutBlock(
      key: key,
      type: type,
      blockType: BlockType.values.firstWhere((e) => e.name == type, orElse: () => BlockType.unknown),
...
void main() {
  group('BlockCard', () {
    testWidgets('shows block type label', (tester) async {
      await tester.pumpWidget(_wrap(BlockCard(block: _makeBlock(type: 'work'))));
      expect(find.text('WORK'), findsOneWidget);
    });

    testWidgets('shows warmup label', (tester) async {
      await tester.pumpWidget(_wrap(BlockCard(block: _makeBlock(type: 'warmup'))));
      expect(find.text('WARMUP'), findsOneWidget);
    });

    testWidgets('shows formatted duration', (tester) async {
      await tester.pumpWidget(_wrap(BlockCard(block: _makeBlock(durationMin: 40.0))));
      expect(find.text('40:00'), findsOneWidget);
    });

    testWidgets('shows power and HR metrics', (tester) async {
      await tester.pumpWidget(_wrap(BlockCard(block: _makeBlock(avgPowerW: 190.0, avgHr: 155.0))));
    ...
    testWidgets('shows compliance bar and percentages when present', (tester) async {
      final compliance = PowerCompliance(belowPct: 10.0, inZonePct: 79.0, abovePct: 11.0);
      final block = _makeBlock(compliance: compliance, targetLow: 188.0, targetHigh: 203.0);
      await tester.pumpWidget(_wrap(BlockCard(block: block)));
